...
-- Single-row SAML 2.0 SSO configuration. Mirrors the branding row pattern:
-- id is a fixed 1, the row is auto-seeded by INSERT IGNORE in §3, and all
-- fields are NULLable so an unconfigured install just renders blank inputs.
-- `enabled` is the master switch ("/saml/* routes work and the login page
-- shows a 'Sign in with SSO' button"). `force_saml` requires `enabled=1`
-- and disables the local /login form unless the bypass file
-- (/data/.saml_bypass) is present — see app/saml.py:bypass_active. The
-- bypass file is the operator's escape hatch when SSO breaks: touching
-- the file re-enables /login until they remove it. `idp_label` controls
-- only the form labels and placeholders on /admin/sso ("Use Okta"
-- toggle); the wire protocol is standard SAML 2.0 either way. The IdP
-- columns hold values the operator pastes from Okta's "View SAML setup
-- instructions" page; the SP columns are auto-filled from the install's
-- request host on first save and rendered as copy-into-Okta hints.
CREATE TABLE IF NOT EXISTS saml_config (
  id INT PRIMARY KEY,
  enabled TINYINT(1) NOT NULL DEFAULT 0,
  force_saml TINYINT(1) NOT NULL DEFAULT 0,
  idp_label ENUM('generic','okta') NOT NULL DEFAULT 'generic',
  idp_entity_id VARCHAR(512),
  idp_sso_url VARCHAR(1024),
  idp_slo_url VARCHAR(1024),
  idp_x509_cert TEXT,
  sp_entity_id VARCHAR(512),
  sp_acs_url VARCHAR(1024),
...
-- 2026-05-09: per-user TOTP enrolment. NULL secret = not enrolled. The
-- secret is a base32 string (160 bits) generated by app/totp.py on
-- enrolment and shown as a QR code one time, after which only the user
-- can recover it from their authenticator app. enrolled_at is set the
-- moment the user verifies a code against the secret (so an abandoned
-- enrolment leaves both columns NULL). auth_source distinguishes local
-- accounts from JIT-provisioned SAML users; the latter cannot use the
-- /login password form even when force_saml is off, since they have no
-- password_hash. Idempotent on reapply.
ALTER TABLE users ADD COLUMN IF NOT EXISTS totp_secret VARCHAR(64) NULL;
ALTER TABLE users ADD COLUMN IF NOT EXISTS totp_enrolled_at DATETIME NULL;
ALTER TABLE users ADD COLUMN IF NOT EXISTS auth_source
  ENUM('local','saml') NOT NULL DEFAULT 'local';
...
-- Single-row SAML config: ensure a default (disabled) row always exists,
-- so the /admin/sso page can render against a stable empty record on
-- first boot without special-casing the missing-row state.
INSERT IGNORE INTO saml_config (id) VALUES (1);
