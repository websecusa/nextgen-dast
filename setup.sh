...
#
# nextgen-dast / pentest-proxy — first-time setup helper.
#
...
APP_URL=${APP_URL:-https://localhost/test/}
...
EOF
    ...
    log "  • APP_URL: ${APP_URL:-https://localhost/test/}  (edit ${ENV_FILE} to change)"
}
...
run_reset() {
    ...
    docker compose --env-file "$ENV_FILE" exec -T pentest-proxy \
...
==============================================================================
  ...
  Application logs:    docker compose --env-file ${ENV_FILE} logs -f pentest-proxy
  Stop the stack:      docker compose --env-file ${ENV_FILE} down

  Admin credentials are in:
      ${secrets_path}

  The application is listening on 127.0.0.1:8888 with root path /test.
