...
_PROBES_NEEDING_POST = {
    ...
    # ----- Round-12 batch (broad-coverage extension) ------------------
    "auth_old_session_after_password_change": "register + change password from one session, replay another",
    "auth_password_reset_token_reuse":      "request reset, consume token, retry with same token",
    "auth_mfa_skip_via_state_param":        "first-factor auth then access dashboard with partial session",
    "auth_mfa_no_lockout_on_codes":         "POST 20 sequential incorrect MFA codes for seed account",
    "authz_resource_owner_field_tamper":    "two fresh accounts + PUT owner_id to other user",
    ...
    "bizlogic_points_negative_redeem":      "POST /redeem with points=-1000",
    "bizlogic_rate_limit_bypass_headers":   "fire past rate limit while rotating XFF/X-Real-IP",
    "bizlogic_subscription_feature_flag_header": "send X-Feature-Flags premium=true on free account",
    "bizlogic_email_header_injection":      "POST forms with CRLF in email field (Bcc injection)",
    "sqli_second_order_via_profile":        "register with SQL canary in bio, GET render endpoints",
    "cmdi_upload_filename_metacharacter":   "upload tiny file with metacharacter in filename",
    "api_content_type_confusion":           "POST privileged JSON fields as XML / form-urlencoded",
    "api_rest_mass_assignment_nested":      "PUT profile with nested account.role=admin object",
    "upload_extension_bypass_double":       "upload benign-content files with double-extension names",
    "upload_svg_xss_or_ssrf":                "upload SVG with script tag and same-origin xlink:href",
    "upload_pdf_external_action":            "upload minimal PDF with /OpenAction /URI",
    "upload_zip_symlink":                    "upload ZIP containing symlink to /etc/hostname",
    "upload_zipslip_traversal":              "upload ZIP with ../ entry and fetch canary path",
    "upload_imagemagick_mvg_canary":         "upload MVG file with same-origin url() callback",
    "upload_decompression_ratio":            "upload high-ratio compressed payload (bounded size)",
    "crypto_client_side_encryption_optional": "JS bundle scan + plaintext POST to login endpoint",
    "auth_remember_me_token_weak":          "POST login with remember-me to harvest persistent cookie",
    "auth_jwt_secret_weak_hmac":            "POST login to harvest JWT, then offline HS256 wordlist crack",
    "auth_jwt_pii_in_claims":               "POST login to harvest JWT, then claims regex sweep",
    "authz_graphql_field_level":            "POST GraphQL queries probing sensitive field-level authz",
    "authz_object_property_field_inflation": "register/login then GET fields= with restricted columns",
    "unicode_homoglyph_login":              "POST login with Cyrillic homoglyph variants of admin",
    "api_batch_endpoint_authz_bypass":      "POST GraphQL batch and REST bulk endpoints",
    "php_type_juggling_magic_hash":         "POST login with PHP magic-hash MD5 candidates",
    "php_array_param_strcmp_bypass":        "POST login with array-shaped username/password params",
}
